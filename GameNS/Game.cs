...
namespace GameNS
{
    ...
    public class Game : IGame, IFileable
    {
        ...
        private void LevelValidation()
        {
            if (GetColumnCount() <= 2)
            {
                string exception = "Width: " + GetColumnCount().ToString();
                throw new ArgumentOutOfRangeException(exception, "Width of level too small");
            }
            if (GetRowCount() <= 2)
            {
                string exception = "Height: " + GetRowCount().ToString();
                throw new ArgumentOutOfRangeException(exception, "Height of level too small");
            }
            if (GetColumnCount() * GetRowCount() < 15)
            {
                string exception = "Units: " + (GetRowCount() * GetRowCount()).ToString();
                throw new ArgumentOutOfRangeException(exception, "Playable area too small");
            }
            if (GetColumnCount() > 18) // 18 dynamic // 7 for manual
            {
                string exception = "Width: " + GetColumnCount().ToString();
                throw new ArgumentOutOfRangeException(exception, "Width of level too large");
            }
            if (GetRowCount() > 11) // 11 dynamic // 7 for manual
            {
                string exception = "Height: " + GetRowCount().ToString();
                throw new ArgumentOutOfRangeException(exception, "Height of level too large");
            }
        }
        ...
        {
            if (!DoesExist(Parts.Goal)
                && !DoesExist(Parts.PlayerOnGoal))
            {
                return true;
            } else { return false; }
        }

        private bool DoesExist(Parts part)
        {
